...
import protomantis
from protomantis import __version__
from protomantis.resources import get_resource_path, PathType
from protomantis.exceptions import UnknownError
...
def parse_console_arguments():
    ...
    parser = argparse.ArgumentParser(prog='protomantis {}'.format(__version__),
                                     description='Visual prototyping tool to design on meshes. Inspired by Rhino's Grasshopper.')
    parser.add_argument('input_file', help='Input file to run protomantis')
    parser.add_argument('output_file', help='Output of protomantis will be written here')
...
def process(input_file: PathType, output_file: PathType, log_file: PathType, status_file: PathType):
    ...
    logging.log(logging.INFO, 'Running protomantis {}'.format(__version__))
